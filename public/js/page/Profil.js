...
class PageProfil {
  ...
  executer() {
    ...
    this._profil.mv = $(".boite_membre table:eq(0) tr:eq(0) td:eq(0)")
      .text()
    ...
    if (!this._profil.estJoueurCourant()) {
      ...
      $(".boite_membre:first div:first table").append(
        `<tr><td class='right'>Retour le :</td><td id='o_tempsRetour'>${moment().add(monProfil.getTempsParcours2(this._profil), "s").format("D MMM à HH[h]mm[m]ss[s]")}</td></tr><tr><td class='right'>Rapport :</td><td id='o_tempsRetourRapport'>${Utils.roundMinute(monProfil.getTempsParcours2(this._profil)).format("D MMM à HH[h]mm")}</td></tr>`,
      );
      Utils.incrementTime(
        monProfil.getTempsParcours2(this._profil),
        "o_tempsRetour",
    ...
    $(".boite_membre:eq(1) table tr td:eq(0)").append(
      `${Utils.comptePlus ? "<br/>" : ""}- <span id='o_surveiller' class='cursor gras'>${this._boiteRadar.joueurs.hasOwnProperty(this._profil.pseudo) ? "Supprimer la surveillance" : "Surveiller ce joueur"}</span>`,
    );
  ...
  plus() {
    ...
    $(".boite_membre:first div:first table").append(
      `<tr><td style='text-align:right'>Temps de trajet :</td><td>${Utils.intToTime(monProfil.getTempsParcours2(this._profil))}</td></tr>`,
    );
    return this;
  }
}
