...
class PageArmee {
  ...
  executer() {
    ...
    $("h3:eq(2)").append(
      ` ${this._nbAttaque}, reste : ${monProfil.niveauRecherche[6] + 1 - this._nbAttaque}.</p>`,
    );
    // Affichage du nombre total d'unité
    $("h3:first").append(
      ` (${numeral(this._armeeTdc.getSommeUnite() + this._armeeDome.getSommeUnite() + this._armeeLoge.getSommeUnite()).format()})</p>`,
    );
    // Bouton antisonde
    ...
    $(".simulateur:first").append(
      "<tr><td colspan=10>Temps <span class='gras' title='Hall Of Fame' >HOF : " +
        Utils.shortcutTime(
          this._armeeTdc.getTemps(0) + this._armeeDome.getTemps(0) + this._armeeLoge.getTemps(0),
        ) +
        "</span>, Temps relatif : <span class='gras'>" +
        Utils.shortcutTime(
          this._armeeTdc.getTemps(monProfil.getTDP()) +
            this._armeeDome.getTemps(monProfil.getTDP()) +
            this._armeeLoge.getTemps(monProfil.getTDP()),
        ) +
        "</span></td></tr>",
    );
  ...
  recupereArmeeTdc() {
    ...
    $(".simulateur tr[align=center]:lt(14)").each((i, elt) => {
      let unite = $(elt).find(".pas_sur_telephone").text(),
        nbr = numeral($(elt).find("td:nth-child(3) span").text()).value();
  ...
  recupereArmeeLoge() {
    ...
    $(".simulateur tr[align=center]:lt(14)").each((i, elt) => {
      let unite = $(elt).find(".pas_sur_telephone").text(),
        nbr = numeral($(elt).find("td:nth-last-child(2)").text()).value();
      if (unite && nbr) unites[unite] = nbr;
    });
  ...
  indicePremiereUnite() {
    ...
    for (let i = 0; i < this._armeeTdc.unite.length; i++)
      if (this._armeeTdc.unite[i] + this._armeeDome.unite[i] + this._armeeLoge.unite[i]) return i;
    return -1;
  }
  /**
   *
  ...
  estPlacePourAntiSonde(indUnite, nbUniteTerrain, nbUniteDome) {
    if (indUnite != -1) {
      ...
      for (let i = 0; i < this._armeeTdc.unite.length; i++) {
        if (this._armeeTdc.unite[i] > 0 && i != indUnite) return false;
        if (this._armeeDome.unite[i] > 0 && i != indUnite) return false;
      }
      // si on assez d'unite on doit respecter les bornes
      if (
        this._armeeTdc.unite[indUnite] +
          this._armeeDome.unite[indUnite] +
          this._armeeLoge.unite[indUnite] >
        nbUniteTerrain + nbUniteDome
      ) {
        // si la premiere unite n'est pas dans les bornes on est mal place
        if (
          this._armeeDome.unite[indUnite] > nbUniteDome ||
          this._armeeDome.unite[indUnite] < nbUniteDome * 0.9 ||
          this._armeeTdc.unite[indUnite] > nbUniteTerrain ||
          this._armeeTdc.unite[indUnite] < nbUniteTerrain * 0.9
        )
          return false;
      }
      return true;
