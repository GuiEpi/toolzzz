...
class PageMessagerie {
  ...
  analyseCombat(id, dateHeure, message) {
    let id_mess = id.split("_")[1],
      combat = new Combat({ id: id_mess, dateHeure: dateHeure, RC: message });
    ...
    $("#" + id + " td:eq(1)").append(
      `<p class="o_optionMessage gras cursor"><span id="show_info_${id_mess}">+</span>${simulation}</p><div id="o_analyse_${id_mess}" class="info_supp separateur_messages_meme_expe" style="display:none">${combat.toHTMLMessagerie()}</div>`,
    );
    ...
    $("#o_simuler_" + id_mess).click((e) => {
      ...
      if (combat.position == 1) {
        ...
        $("#o_bouclier2").spinner(
          "value",
          combat.attaquant.niveauRecherche[1] != -1 ? combat.attaquant.niveauRecherche[1] : 0,
        );
      } else {
        $("#o_armes2").spinner("value", combat.defenseur.niveauRecherche[2]);
        $("#o_bouclier2").spinner(
          "value",
          combat.defenseur.niveauRecherche[1] != -1 ? combat.defenseur.niveauRecherche[1] : 0,
        );
  ...
  analyseChasse(id_conv, id, message) {
    ...
    if (!this._messagesOuvert.hasOwnProperty(id_mess)) {
      ...
      chasse.analyse();
      $("#" + id + " td:eq(1)").append(
        `<p id="show_info_${id_mess}" class="gras cursor">+</p><div id="o_analyse_${id_mess}" class="info_supp separateur_messages_meme_expe" style="display:none">${chasse.toHTMLMessagerie()}</div>`,
      );
      $("#show_info_" + id_mess).click((e) => {
        $(e.currentTarget)
          .text($(e.currentTarget).text() == "+" ? "-" : "+")
      ...
      this._messagesOuvert["conv_" + id_conv] = this._messagesOuvert.hasOwnProperty(
        ...
        ? this._messagesOuvert["conv_" + id_conv].ajoute(chasse)
        : chasse;
    }
    return this;
  }
  ...
  analyseChasses(id_conv) {
    if (!$("#o_bilan_" + id_conv).length) {
      $("#conversation_" + id_conv)
        ...
        .find(".message:last")
        .append(
          `<p id="show_bilan_${id_conv}" class="gras cursor souligne">Bilan</p><div id="o_bilan_${id_conv}" class="info_supp separateur_messages_meme_expe" style="display:none">${this._messagesOuvert["conv_" + id_conv].toHTMLMessagerie()}</div>`,
        );
  ...
  couleurMessageUtilitaire() {
    $("tr[id^='conversation_']").each((i, elt) => {
      ...
      // Invasion de xXx: votre armée
        ...
        color = this._utilitaire.alliance.joueurs.hasOwnProperty(titre.split(" ")[2])
          ? "green"
          : "red";
