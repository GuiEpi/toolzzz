  ...
  event() {
    ...
    $("input[name^='o_nombre']").on("input spin", (e, ui) => {
      ...
      this.majTemps(
        ...
        nombre *
          (TEMPS_UNITE[
            NOM_RAC_UNITE.indexOf($(e.currentTarget).closest("tr").find("td:first").text())
          ] *
    ...
    $("#o_niveauTDP").on("input spin", (e, ui) => {
      ...
      $("input[name^='o_nombre']").each((i, elt) => {
        ...
        $(elt)
          ...
          .parent()
          .prev()
          .text(BoitePonte.arrondiTemps(TEMPS_UNITE[unite] * Math.pow(0.9, tdp)));
    ...
    $("img[id^=o_lancer]").click((e) => {
      let unite = ~~$(e.currentTarget).attr("id").replace("o_lancer", ""),
        nombre = $("input[name='o_nombre" + unite + "']").spinner("value"),
        securite = "";
