...
class BoiteRang extends Boite {
  constructor(joueur, utilitaire, page) {
    super(
      "o_boiteRang" + joueur.id,
      "Attribuer un rang",
      `<form id="o_form${joueur.id}" class="o_rangForm">
   ...
   */
  afficher() {
    if (super.afficher()) this.css().event();
    return this;
  }
  /**
  ...
  event() {
    ...
    $("#o_form" + this._joueur.id + " button[name='o_btnRang']").click((e) => {
      ...
      this._utilitaire
        .modifierSujet(this._joueur.toUtilitaire(), " ", this._joueur.sujetForum)
        .then(
          (data) => {
