...
class BoiteCommande extends Boite {
   ...
   */
  afficher() {
    if (super.afficher()) this.getForm().css().event();
    return this;
  }
  /**
  ...
  event() {
    ...
    if (!this._utilitaire.commande.hasOwnProperty(this._commande.id))
      $("#o_form" + this._commande.id + " select[name='o_evolution']").change((e) => {
        ...
        $("#o_form" + this._commande.id + " input[name='o_quantiteNou']").val(
          numeral(qte[0]).format(),
        );
        $("#o_form" + this._commande.id + " input[name='o_quantiteMat']").val(
    ...
    $("#o_commander" + this._commande.id).click((e) => {
      e.preventDefault();
      this._commande.evolution = $(
        "#o_form" + this._commande.id + " select[name='o_evolution']",
      ).val();
      this._commande.nourriture = numeral(
        $("#o_form" + this._commande.id + " input[name='o_quantiteNou']").val(),
      ).value();
      this._commande.materiaux = numeral(
        $("#o_form" + this._commande.id + " input[name='o_quantiteMat']").val(),
      ).value();
      this._commande.dateSouhaite = moment(
        $("#o_form" + this._commande.id + " input[name='o_dateCommande']").val(),
        "DD-MM-YYYY",
      );
      let dateApres = $("#o_form" + this._commande.id + " input[name='o_dateApres']").val();
      this._commande.dateApres = dateApres
        ? moment(
            $("#o_form" + this._commande.id + " input[name='o_dateApres']").val(),
            "DD-MM-YYYY",
          )
        : null;
      ...
      if (!message) {
        ...
        if (!this._utilitaire.commande.hasOwnProperty(this._commande.id)) {
          ...
          this._utilitaire
            .modifierSujet(this._commande.toUtilitaire(), " ", this._commande.id)
            .then(
              (data) => {
  ...
  getForm() {
    let select = "",
      qte = Utils.calculQuantite(0);
    for (
      let i = 0;
      i < EVOLUTION.length;
      select += `<option value="${i}" ${i == this._commande.evolution ? "selected" : ""}>${EVOLUTION[i++]}</option>`
    );
