...
class Combat {
  ...
  calculerBouclierLieu(lieu, degat, armee1, armee2, armes) {
    let solution = new Array(),
      viePerdue = armee1.getBaseVie() - armee2.getBaseVie(),
      tmpLieu = -1;
    switch (lieu) {
      case 1:
        for (let bouclier = armes - 3; bouclier <= armes + 2; bouclier++) {
          tmpLieu = ((degat - viePerdue) / viePerdue) * 20 - 2 - 2 * bouclier;
          if (tmpLieu <= 45 && tmpLieu >= 0) solution.push(bouclier + "/" + Math.round(tmpLieu));
        }
        break;
      case 2:
        for (let bouclier = armes - 3; bouclier <= armes + 2; bouclier++) {
  ...
  calculerTDP(armee) {
    ...
    for (let i = 0; i < 140; i++) {
      ...
      if (Math.abs(tempsPonte - 60) < 0.01 || tempsPonte < 0.01) tmpTDP.push(i);
    }
    return tmpTDP;
  }
  /**
      ...
      if (this._pointDeVue == 1) {
        ...
        if (this._rc.split("et tuez").length > 2 || this._rc.indexOf("Vous avez gagné") == -1) {
          if (this._rc.includes("attaque votre") || this._lieu == LIEU.TERRAIN)
            this._attaquant.niveauRecherche[1] = this.calculerBouclier(
              base1 + bonus1,
              this._armeeEnnemieAv,
              this.retirerPerte(this._armeeEnnemieAv, "et tuez", 2),
            );
          else
            this._attaquantBonusLieu = this.calculerBouclierLieu(
              this._lieu,
              base1 + bonus1,
              this._armeeEnnemieAv,
              this.retirerPerte(this._armeeEnnemieAv, "et tuez", 2),
              this._defenseur.niveauRecherche[2],
            );
        }
        ...
        if (this._rc.split("et en tue").length > 2 || this._rc.includes("Vous avez gagné")) {
          if (!this._rc.includes("attaque votre") || this._lieu == LIEU.TERRAIN)
            this._defenseur.niveauRecherche[1] = this.calculerBouclier(
              base2 + bonus2,
              this._armeeAv,
              this.retirerPerte(this._armeeAv, "et en tue", 2),
            );
          else
            this._defenseurBonusLieu = this.calculerBouclierLieu(
              this._lieu,
              base2 + bonus2,
              this._armeeAv,
              this.retirerPerte(this._armeeAv, "et en tue", 2),
              this._attaquant.niveauRecherche[2],
            );
        }
      } else {
        let tmp1 = this._rc.split("Vous infligez")[1].split("dégâts")[0],
  ...
  retirerPerte(armee, separateur, nbTour = 100000) {
    ...
    for (
      let i = 1, tmp = this._rc.split(separateur);
      i < Math.min(tmp.length, nbTour);
  ...
  toHTMLBoite() {
    ...
    if (this._armeePe.getBaseAtt() != this._armeeAp.getBaseAtt()) {
      let diff1 = this._armeeAp.getBaseVie() - this._armeeAv.getBaseVie(),
        diff2 = this._armeeAp.getBaseAtt() - this._armeeAv.getBaseAtt(),
        diff3 = this._armeeAp.getBaseDef() - this._armeeAv.getBaseDef();
  ...
  simuler() {
    ...
    let vieAttaquant = new Array().fill(0),
      vieDefenseur = new Array().fill(0);
    this._armeeAv.unite.forEach((elt, i) => {
      vieAttaquant[i] = this.calculerVieUnite(elt, i + 1, this._attaquant.niveauRecherche[1], 0, 0);
    });
    this._armeeEnnemieAv.unite.forEach((elt, i) => {
      vieDefenseur[i] = this.calculerVieUnite(
        elt,
        i + 1,
        this._defenseur.niveauRecherche[1],
        this._lieu,
        this._lieu == LIEU.TERRAIN
          ? 0
          : this._lieu == LIEU.DOME
            ? this._defenseur.niveauConstruction[9]
            : this._defenseur.niveauConstruction[10],
      );
    });
    // on repique à 10% si l'attaque est strictement supérieur à la vie en def
    let replique = this.calculerReplique(
      this._armeeAv.getTotalAtt(this._attaquant.niveauRecherche[2]),
      vieDefenseur,
    );
    // tant qu'il rete de la vie sur la defense ou l'attaque on enchaine les tours
    while (
      vieAttaquant.reduce((acc, val) => {
        return acc + val;
      }, 0) > 0 &&
      vieDefenseur.reduce((acc, val) => {
        return acc + val;
      }, 0) > 0
    ) {
      baseDegatAtt = this._attaquant.niveauRecherche[2]
        ? this._armeeAp.getBaseAtt()
        : Math.ceil(this._armeeAp.getBaseAtt());
      ...
      baseDegatDef = this._defenseur.niveauRecherche[2]
        ? this._armeeEnnemieAp.getBaseDef() * replique
        : Math.ceil(this._armeeEnnemieAp.getBaseDef() * replique);
      ...
      retourVieTmp = this.retirerVie(
        this._armeeEnnemieAp,
        vieDefenseur,
        baseDegatAtt + bonusDegatAtt,
      );
      ...
      this._tour.push([
        ...
        this._armeeEnnemieAp.getSommeUnite() - armeeDefTmp.getSommeUnite(),
        Math.ceil(baseDegatDef),
        Math.ceil(bonusDegatDef),
        this._armeeAp.getSommeUnite() - armeeAttTmp.getSommeUnite(),
      ]);
  ...
  calculerReplique(pointAtt, pointVie) {
    if (pointAtt > pointVie) return 0.1;
    return 1;
  }
  /**
   *
  ...
  retirerVie(armee, vieUnite, degatInflige) {
    ...
    for (let i = 0; i < armeePerdu.unite.length; i++) {
      if (armeePerdu.unite[i]) {
        // si on a des unités
        if (vieUnite[i] >= degatInflige) {
  ...
  calculerVieUnite(nombre, unite, bonusBouclier, lieu, bonusLieu) {
    let vie =
      nombre * VIE_UNITE[unite] + Math.round((nombre * VIE_UNITE[unite] * bonusBouclier) / 10);
  ...
  calculeUniteXP(armee, ratio) {
    let ordreXp = new Array(0, 1, 3, 4, 5, 7, 10, 12),
      armeeXP = new Armee();
    ordreXP.forEach((val, i) => {
      armeeXP.unite[val] = Math.round(armee.unite[val] * ratio);
    });
    return armee;
  }
  /**
   ...
   */
  genererRC() {
    let boiteRC = new BoiteRapport(
      this._id,
      this._pointDeVue == 0 ? this.genererRCAttaquant() : this.genererRCDefenseur(),
    );
    boiteRC.afficher();
    return this;
  }
