...
class Commande {
  constructor(parametres = {}) {
     ...
     */
    this._demandeur = parametres.hasOwnProperty("demandeur")
      ? new Joueur(parametres["demandeur"])
      : monProfil;
    /**
  ...
  toUtilitaire() {
    return (
      "[" +
      Object.keys(ETAT_COMMANDE).find((key) => ETAT_COMMANDE[key] == this._etat) +
      "] " +
      this._demandeur.x +
      " / " +
      this._demandeur.y +
      ...
      this._nourriture +
      " / " +
      moment(this._dateSouhaite).format("D MMM YYYY") +
      (this._dateApres ? " / " + moment(this._dateApres).format("D MMM YYYY") : "")
    );
  }
  /**
   *
  ...
  estAFaire() {
    return !(
      ...
      this._etat == ETAT_COMMANDE["Annulée"] ||
      this._etat == ETAT_COMMANDE["Terminée"] ||
      (this._etat == ETAT_COMMANDE["Nouvelle"] && this._demandeur.pseudo != monProfil.pseudo)
    );
  }
  /**
   *
   */
  estValide() {
    if (this._nourriture && this._nourriture < 0) return "Quantité nourriture incorrecte.";
    if (this._materiaux && this._materiaux < 0) return "Quantité materiaux incorrecte.";
    if (!this._dateSouhaite.isValid()) return "Date de la demande invalide.";
  ...
  ajouterEvent(page, utilitaire) {
    $("#o_commande" + this._id).click((e) => {
      let livraison = Math.floor(
          (Utils.ouvrieres - Utils.terrain) * (10 + monProfil.niveauConstruction[11] / 2),
        ),
      ...
      if (this._materiaux < max) {
        ...
        max -= this._materiaux;
        $("#input_nbNourriture").val(
          numeral(this._nourriture < max ? this._nourriture : max).format(),
        );
    ...
    $("#o_supprimerCommande" + this._id).click((e) => {
      if (confirm("Supprimer cette commande ?")) {
        this._etat = ETAT_COMMANDE.Supprimée;
        utilitaire.modifierSujet(this.toUtilitaire(), " ", this._id).then(
          (data) => {
