...
- **Ratio** (quatrième valeur de l'option précédente) — répartit _toutes_ les ouvrières entre nourriture et matériaux selon la **Part en nourriture** choisie au curseur (par pas de 10 %, le reste va aux matériaux). Contrairement aux modes Matériaux / Nourriture qui ne font que compléter les ouvrières inutilisées, le ratio est réappliqué à chaque consultation de la page Ressources : la répartition est donc conservée après une chasse, un flood ou une perte de terrain. Le curseur est aussi disponible directement sur la page Ressources, où un changement s'applique immédiatement.
